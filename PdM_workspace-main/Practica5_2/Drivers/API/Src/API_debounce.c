 ...
 * @param  None
 * @retval true if the button is pressed, false if it is released*/
bool_t readKey(){
	bool_t flag = false;
	if (isPressed) {
		flag = true;
...
/* @brief: print in the uart falling edge
 * @param: none
 * retval: none*/
void fallingEdge(){
...
/* @brief: print in the uart rising edge
 * @param: none
 * retval: none*/
void risingEdge(){
		...
		case BUTTON_FALLING:
			...
			if(delayRead(&delay)){
				...
				if(BSP_PB_GetState(BUTTON_USER)) {
					...
					isPressed = true;
					/* Show in serial console that a falling edge was detected */
					fallingEdge();
				}
				else{
		...
		case BUTTON_RISING:
			...
			if(delayRead(&delay)){
				if(!BSP_PB_GetState(BUTTON_USER)){
					...
					isPressed = false;
					/* Show in serial console that a rising edge was detected */
					risingEdge();
				}
				else{
					/* If spurious noise detected, return to previous state */
